...
```
method(formula, data, options) # usually require tidy data 
```
...
```{r}
predict(my_lm, newdata = tibble(gdpPercap = c(500, 600, 700))) %>% #predicts y values for x values i.e. predicting lifeExp for each log(gdpPercap) value, uses all of gapminder if newdata is not specified
  head()
```
...
```{r}
tidy(my_lm)
augment(my_lm) # can specifiy new data as with the predict function
glance(my_lm)
```
