...
These functions we've seen are called __vectorized functions__. Operate independently on every component of vector

## `summarize()` and `group_by()`

Use `summarize()` to compute the mean and median life expectancy using all entries:
`summarize()` takes vector and returns one value
