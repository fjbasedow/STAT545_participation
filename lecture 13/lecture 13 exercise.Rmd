...
p <- ggplot(mtcars, aes(hp, wt)) + 
    geom_point()
ggsave("my_plot.png", p) #first part includes directory, but should start in location of Rmd file
```
...
p_scales + 
    scale_x_log10() +
    scale_colour_continuous(trans="log10") # population on log scale
```
...
p_scales +
    ...
    scale_y_continuous(breaks=1:10 * 10) #ggplot drops the ones out of range
```
...
#p %>% 
 #   ggplotly() %>% 
  #  htmlwidgets::saveWidget("LOCATION_GOES_HERE") #saves as html file
```
...
#p %>% 
 #   ggplotly() %>% 
  #  plotly_json() #underlying plotly is plain text
```
