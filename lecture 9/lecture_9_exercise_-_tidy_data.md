...
tibble(let = letters, 
       ...
       comb = paste(let, num)) # paste0() pastes without space in between
```
...
``` r
write_csv(iris, "iris.csv") #prints csv of iris in current working directory
```

Write the `iris` data frame to a file delimited by a dollar sign.

``` r
write_delim(iris, "iris.txt", delim="$") # separates value by dollar sign
```
...
``` r
gather(lotr_untidy, key="Gender", value="Word", Female, Male) # similar to melt function from reshape2
```
...
``` r
gather(lotr_untidy, key="Gender", value="Word", Female:Male) #last part specifies levels of categorical variable
```
...
``` r
lotr_list <- (list(lotr1, lotr2, lotr3)) #makes list that has 3 components, each of which is a data frame
lotr_list
```
...
``` r
lotr_tidy %>% 
  unite(Race_Gender, Race, Gender) %>% #first argument new name of column, second and thrid, variables you want to unite
...
```
    ...
    ##    <chr>                      <chr>  <int>   <dbl>   <dbl>
    ##  1 The Fellowship Of The Ring Elf      971  NA      -0.909
    ##  2 The Fellowship Of The Ring Elf     1229  -0.851  NA    
    ##  3 The Fellowship Of The Ring Hobbit    14  -0.789  NA    
    ##  4 The Fellowship Of The Ring Hobbit  3644  NA      -0.400
    ##  5 The Fellowship Of The Ring Man        0  -0.172  NA    
    ##  6 The Fellowship Of The Ring Man     1995  NA       0.336
    ##  7 The Return Of The King     Elf      183  -0.836  NA    
    ##  8 The Return Of The King     Elf      510  NA      -0.460
    ##  9 The Return Of The King     Hobbit     2  -0.942  NA    
    ## 10 The Return Of The King     Hobbit  2673  NA       1.49 
    ## 11 The Return Of The King     Man      268   1.69   NA    
    ## 12 The Return Of The King     Man     2459  NA       0.242
    ## 13 The Two Towers             Elf      331   0.410  NA    
    ## 14 The Two Towers             Elf      513  NA       0.928
    ## 15 The Two Towers             Hobbit     0   2.07   NA    
    ## 16 The Two Towers             Hobbit  2463  NA      -2.14 
    ## 17 The Two Towers             Man      401  -0.423  NA    
    ## 18 The Two Towers             Man     3589  NA       1.71
...
``` r
expand(mtcars, nesting(vs, cyl)) # makes every possible combination of values in the two variable vs and cyl
```
...
```
    ...
    ##   <chr>  <chr> <dbl>        <dbl>        <dbl>
    ## 1 Alex   a        1.        0.496       0.0228
    ## 2 Alex   a        2.        0.926       0.488 
    ## 3 Alex   a        3.        0.775       0.911 
    ## 4 Robert b        1.        0.892       0.648 
    ## 5 Robert b        2.        0.324       0.863 
    ## 6 Sam    a        1.        0.237       0.168
...
``` r
# drop_na will drop rows with NAs
# replace_na will replace NAs with what you want 
```
